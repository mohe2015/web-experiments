// professionally just ignoring the disclaimer at https://developer.mozilla.org/en-US/docs/Web/API/Web_Crypto_API
// https://developer.mozilla.org/de/docs/Web/API/SubtleCrypto
...
function getMessageEncoding() {
  let message = (document.querySelector("#pbkdf2-message") as HTMLInputElement).value;
...
function getKeyMaterial() {
  let password = window.prompt("Enter your password") as string;
  let enc = new TextEncoder();
  // @ts-ignore
...
async function encrypt() {
  const ciphertextValue = document.querySelector(".pbkdf2 .ciphertext-value") as HTMLElement;
  ciphertextValue.textContent = "";
  const decryptedValue = document.querySelector(".pbkdf2 .decrypted-value") as HTMLElement;
